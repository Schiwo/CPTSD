...
def calculate_distances(filename):
    ...
    for index, row in data_filtered.iterrows():
        if pd.isna(row["Section"]):
            ...
            data_filtered.at[index, "Section"] = [  re.sub(r'["\']', '', item.strip()) for item in row["Section"].split("/")]
        if pd.isna(row["Estimated Section"]):
            print("Test")
            data_filtered.at[index, "Estimated Section"] = [None]
        else:
            data_filtered.at[index, "Estimated Section"] = [re.sub(r'["\']', '', item.strip()) for item in row["Estimated Section"].split("/")]
    
    data_long = pd.DataFrame(columns=["StatementNr", "Statement", "Section", "Estimated Section", "d", "overlap"])
    for index, row in data_filtered.iterrows():

        # Iterate over the lists in "Section" and "Estimated Section"
        statement = row["Statement"]
        tokenized = tokenizer(statement, return_offsets_mapping=True, add_special_tokens=False)
        ...
        if not None in estimated_sections:
            estimated_indices = [get_token_indices(section, statement, offsets) for section in estimated_sections]
        else:
        ...
        for ground_truth in row["Section"]:
            ...
            sum_of_estimated_tokens = {token for section in estimated_indices for token in section}
            overlap = len(set(ground_truth_indices) & sum_of_estimated_tokens) / len(set(ground_truth_indices))
            ...
            print(f"The overlap of the ground truth with all estimated sections is: {overlap}")
